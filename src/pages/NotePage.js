...
const NotePage = (props) => {
    ...
    const takeNote = async () => {
        await props.createNote(props.match.params.id , note)
        setNote("")
    }

    const updateNote = (note) => {
        props.updateNote(props.match.params.id , note)
    }
        ...
        <div>
            <Header height="80px" backgroundColor="#151b26" isFloat={true} shadow={true} >
                ...
                <HeaderZone position={'right'} size={4} paddingRight="50px">
                    <HeaderItem justifyContent={"start"} >
                        <Link to="/" style={{textDecoration:"none" , color:"white" , paddingRight:"30px"}}>collections</Link>
                    </HeaderItem>
                    <HeaderItem justifyContent={"start"} >
            ...
            <div style={{ padding:"8% 10% 0px" , minHeight:"100vh" , backgroundColor:"#f6f8f9"}}>
                ...
                <div style={{display:"flex" , flexDirection:"column" }}>
                    ...
                    <div style={{margin:"24px 0"}}>
                        <NoteList deleteNote={(note) => props.deleteNote(props.match.params.id, note)} notes={notes} update={updateNote} />
                    </div>
